...
} from '@/components/ConsultationNotesModal'
import './page.css'
...
export default function ConsultationNotesDemoPage() {
  ...
  const mockExistingNotes: ConsultationNote = {
    ...
    time: '10:30 - 11:15',
    sessionSummary:
      'Phiên tư vấn tập trung vào việc thảo luận về tình trạng lo âu và stress trong công việc. Khách hàng thể hiện sự cải thiện đáng kể so với các phiên trước, có khả năng nhận diện và quản lý cảm xúc tốt hơn.',
    keyObservations:
      'Khách hàng có dấu hiệu tích cực hơn trong giao tiếp, tự tin hơn khi chia sẻ. Vẫn còn một số lo lắng về hiệu suất công việc nhưng đã phát triển được các phương pháp đối phó hiệu quả như kỹ thuật thở sâu và tư duy tích cực.',
    recommendations:
      'Tiếp tục thực hiện các kỹ thuật mindfulness đã học, đặc biệt là bài tập thở sâu trước các cuộc họp quan trọng. Tăng cường hoạt động thể chất ít nhất 30 phút/ngày và duy trì lịch ngủ ổn định từ 22:30-6:30.',
    nextSteps:
      'Theo dõi tiến triển trong 2 tuần tới thông qua nhật ký cảm xúc hàng ngày. Khách hàng sẽ thực hiện bài tập CBT về tái cấu trúc tư duy tiêu cực. Lên kế hoạch thảo luận về mối quan hệ gia đình trong phiên tiếp theo.',
    riskLevel: 'low',
    followUpDate: '2026-09-09',
    confidentialNotes:
      'Khách hàng đã chia sẻ về những khó khăn trong mối quan hệ với cấp trên và áp lực từ gia đình về việc thăng tiến. Cần tiếp tục hỗ trợ trong việc xây dựng ranh giới cá nhân và kỹ năng giao tiếp assertive.',
