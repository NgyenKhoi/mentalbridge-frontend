...
const QUESTIONS = [
  {
    ...
    placeholder: 'Một cuộc trò chuyện, sự kiện hoặc suy nghĩ...',
  },
  {
    id: 'physical',
    label: 'Cơ thể bạn phản ứng như thế nào?',
    placeholder: 'Căng ở vai, tim đập nhanh, mệt mỏi...',
  },
  {
    id: 'thought',
    label: 'Bạn đang nghĩ gì lúc này?',
    placeholder: 'Những suy nghĩ đang lặp lại trong đầu bạn...',
  },
  {
    id: 'need',
    label: 'Điều bạn cần lúc này là gì?',
    placeholder: 'Được lắng nghe, thời gian một mình, một cái ôm...',
  },
]
...
export default function JournalDetailModal({
  ...
  const formatDate = (dateValue: string) =>
    new Date(`${dateValue}T12:00:00`).toLocaleDateString('vi-VN', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric',
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            className="journal-detail-backdrop"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => mode === 'view' && !isWorking && onClose()}
          />
          <motion.div className="journal-detail-shell">
            ...
            >
              <AnimatePresence mode="wait" initial={false}>
                {mode === 'delete' ? (
                  <motion.div
                    className="journal-delete-confirm"
                    key="delete"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                  >
                    <span className="journal-danger-kicker">
                      Xác nhận thao tác
                    </span>
                    <div className="delete-icon" aria-hidden="true">
                      ...
                      >
                        <path
                          d="M3 6h18M8 6V4h8v2m3 0-1 15H6L5 6m5 4v7m4-7v7"
                          strokeLinecap="round"
                    ...
                    <p>
                      Nhật ký ngày <strong>{formatDate(entry.date)}</strong> sẽ
                    ...
                    <div className="delete-actions">
                      ...
                      <button
                        ...
                        disabled={isWorking}
                      >
                        {isWorking ? 'Đang xóa...' : 'Xóa vĩnh viễn'}
                      </button>
                    </div>
                  </motion.div>
                ) : (
                  <motion.div
                    key={mode}
                    initial={{ opacity: 0, x: mode === 'edit' ? 16 : -8 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: -8 }}
                  >
                    <header className="journal-detail-header">
                      <div className="journal-detail-heading">
                        <span className="journal-detail-eyebrow">
                          {mode === 'edit'
                            ? 'Cập nhật nhật ký'
                            : 'Chi tiết nhật ký'}
                        </span>
                        <div className={`journal-detail-mood ${mood.tone}`}>
                          <span className="mood-emoji" aria-hidden="true">
                            {mood.emoji}
                          </span>
                          <div>
                            <h2 id="journal-detail-title">{mood.label}</h2>
                            <time>
                              {formatDate(draft.date)} <span>·</span>{' '}
                              {draft.time}
                            </time>
                          </div>
                        </div>
                      ...
                      <button
                        ...
                        aria-label={mode === 'edit' ? 'Hủy chỉnh sửa' : 'Đóng'}
                      >
                        ×
                      </button>
                    </header>
                    ...
                    {mode === 'view' ? (
                      <div className="journal-detail-content">
                        <section className="journal-detail-section journal-note-section">
                          <span className="detail-section-number">01</span>
                          <div>
                            <h3>Ghi chú trong ngày</h3>
                        ...
                        {draft.psychologyAnswers &&
                          Object.values(draft.psychologyAnswers).some(
                            Boolean,
                          ) && (
                            <section className="journal-detail-section">
                              ...
                              <div className="detail-section-body">
                                <h3>Nhìn sâu hơn</h3>
                                <div className="psychology-answers">
                                  {QUESTIONS.filter(
                                    (question) =>
                                      draft.psychologyAnswers?.[question.id],
                                  ).map((question) => (
                                    <div
                                      key={question.id}
                                      className="psychology-answer-item"
                                    >
                      ...
                      <div className="journal-edit-content">
                        <fieldset className="journal-edit-moods">
                          <legend>Cảm xúc của bạn</legend>
                          <div>
                            {MOODS.map((item) => (
                              <button
                                type="button"
                                key={item.value}
                                className={
                                  draft.mood === item.value ? 'selected' : ''
                                }
                                onClick={() =>
                                  setDraft({ ...draft, mood: item.value })
                                }
                        ...
                        <label className="journal-edit-field">
                          ...
                          />
                          <small>{draft.content.length}/1000</small>
                        </label>
                        {error && (
                          <p className="journal-edit-error" role="alert">
                            {error}
                          </p>
                        )}
                        <div className="journal-edit-reflection">
                          <div>
                            <span aria-hidden="true">✦</span>
                            <div>
                              <h3>Khám phá sâu hơn</h3>
                              <p>
                                Bạn có thể bỏ trống những câu chưa muốn trả lời.
                              </p>
                            </div>
                          </div>
                        ...
                        <label className="journal-edit-field journal-tag-field">
                          <span>Chủ đề</span>
                          <input
                            value={tagText}
                            onChange={(event) => setTagText(event.target.value)}
                            placeholder="work, exercise, sleep"
                          />
                    ...
                    <footer className="journal-detail-actions">
                      {mode === 'view' ? (
                        <>
                          <button
                            className="detail-btn detail-btn-danger-text"
                          ...
                          >
                            ...
                            >
                              <path
                                d="m4 16-1 5 5-1L20 8l-4-4L4 16Z"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                              />
                              <path d="m14 6 4 4" />
