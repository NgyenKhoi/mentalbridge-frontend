...
const PLAN_LABEL: Record<PlanId, string> = { free: 'Miễn phí', plus: 'Plus', premium: 'Premium' }
...
export default function PlanGate({
  ...
  return (
    <div className="plan-gate" data-required-plan={required}>
      <div className="plan-gate-preview" aria-hidden="true">{children}</div>
      <div className="plan-gate-overlay">
        <span className={`plan-gate-badge ${required}`}>{PLAN_LABEL[required]}</span>
        <strong>{title}</strong>
        <p>{description}</p>
        <Link href="/subscription" className="plan-gate-action">Nâng cấp {PLAN_LABEL[required]} <span aria-hidden="true">→</span></Link>
      </div>
    </div>
  )
}
