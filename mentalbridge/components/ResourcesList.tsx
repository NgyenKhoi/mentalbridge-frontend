...
type LoadingState = 'idle' | 'loading' | 'success' | 'error' | 'empty' | 'timeout' | 'unavailable';
...
export default function ResourcesList({ 
  ...
  if (loadingState === 'loading') {
    return (
      <div className={`resources-list ${className}`}>
        ...
        <div className="resources-grid" style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
          {[...Array(3)].map((_, i) => (
            <div 
              key={i}
              style={{
  ...
  if (loadingState === 'error' || loadingState === 'timeout' || loadingState === 'unavailable') {
    return (
      <div className={`resources-list ${className}`}>
        <div 
          style={{
        ...
        >
          <svg 
            ...
            stroke="currentColor" 
            strokeWidth="1.5"
            style={{ margin: '0 auto 1rem', opacity: 0.6, color: 'var(--amber)' }}
          >
    ...
    >
      ...
      <div style={{
        ...
        <p style={{ fontSize: '0.9rem', textAlign: 'center' }}>
          <strong>Sắp có:</strong> Đăng ký nhận tư vấn chuyên gia và tham gia nhóm hỗ trợ
          <span style={{ 
            marginLeft: '0.5rem',
            fontSize: '0.8rem',
