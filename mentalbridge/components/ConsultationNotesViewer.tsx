...
export default function ConsultationNotesViewer({
  ...
  const getRiskLevelDisplay = (level: string) => {
    switch (level) {
      case 'high':
        return { text: 'Cao - Cần can thiệp khẩn cấp', className: 'high' }
      case 'medium':
        return { text: 'Trung bình - Cần theo dõi', className: 'medium' }
      default:
        return { text: 'Thấp - Ổn định', className: 'low' }
    }
  }
              ...
              <div className="consultation-notes-sections">
                ...
                {notes.followUpDate && (
                  <div className="consultation-notes-section">
                    ...
                    <p>
                      {new Date(notes.followUpDate).toLocaleDateString(
                        'vi-VN',
                        {
                          weekday: 'long',
                          day: 'numeric',
