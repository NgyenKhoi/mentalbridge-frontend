...
export function BreathingIntro({ onFinish }: BreathingIntroProps) {
  ...
  useEffect(() => {
    ...
    const languageTimer = reducedMotion ? undefined : window.setTimeout(() => setLanguage('en'), 1700)
  ...
  return (
    <div className={`breathing-intro${leaving ? ' breathing-intro--leaving' : ''}`} data-lenis-prevent="" role="dialog" aria-modal="true" aria-label="A quiet breathing moment / Khoảnh khắc hít thở cùng MentalBridge">
      <div className="breathing-intro__stars" aria-hidden="true" />
      <button className="breathing-intro__skip" type="button" onClick={finish}>Bỏ qua</button>
      ...
      <div className="breathing-intro__content">
        <div className="breathing-intro__orb" aria-hidden="true">
          ...
          <span className="breathing-intro__wave"><i /><i /><i /><i /><i /></span>
        </div>
        ...
        <div className="breathing-intro__phase" aria-live="polite" aria-atomic="true">
          <span key={`${language}-${phase}`}>
            <strong>{language === 'vi'
              ? (phase === 'inhale' ? 'Hít vào' : 'Thở ra')
              : (phase === 'inhale' ? 'Inhale gently' : 'Exhale softly')}</strong>
          </span>
        </div>
