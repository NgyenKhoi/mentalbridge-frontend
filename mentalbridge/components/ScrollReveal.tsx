...
export default function ScrollReveal() {
  useEffect(() => {
    ...
    media.add('(prefers-reduced-motion: no-preference)', () => {
      ...
      revealGroup('.value-item', '.value-row', { y: 32, opacity: 0 }, .09)
      revealGroup('[data-group="barrier"]', '.barrier-grid', { x: -48, y: 24, opacity: 0, scale: .94, rotationY: 6 }, .1)
      revealGroup('.journey-step', '.journey-path', { y: 42, opacity: 0, scale: .96 }, .09)
      // The tools enter as a soft, staggered stack instead of appearing all at once.
      revealGroup('.service-row', '.services-list', { x: -30, y: 12, opacity: 0 }, .08)
      revealGroup('[data-group="risk"]', '.risk-grid', { y: 54, opacity: 0 }, .09)
      revealGroup('.footer-top > *', '.footer-top', { y: 30, opacity: 0 }, .075)
          ...
          scrollTrigger: {
            ...
            onUpdate: self => {
              journeySteps.forEach((step, index) => {
                const threshold = journeySteps.length > 1 ? index / (journeySteps.length - 1) : 0
                step.classList.toggle('on', self.progress + .02 >= threshold)
              })
            },
          },
        })
      ...
      if (cta) {
        consumed.add(cta)
        const children = Array.from(cta.querySelectorAll<HTMLElement>('h2,p,a'))
        children.forEach(child => consumed.add(child))
        gsap.from(children, {
          y: 32,
          opacity: 0,
