...
const heroTopics = [
  { label: 'Lo âu', eyebrow: 'Lắng nghe điều đang diễn ra', title: 'Bạn có đang\nổn không?', description: 'Nhận diện cảm xúc và tìm đúng sự hỗ trợ khi bạn cần.' },
  { label: 'Giá trị bản thân', eyebrow: 'Bạn vẫn luôn đủ đầy', title: 'Nhẹ nhàng hơn với chính mình.', description: 'Ghi lại cảm xúc, nhìn thấy tiến triển và xây dựng lòng trắc ẩn với bản thân mỗi ngày.' },
  { label: 'Các mối quan hệ', eyebrow: 'Kết nối bằng sự thấu hiểu', title: 'Hiểu mình để gần nhau hơn.', description: 'Nhận diện điều khó nói và kết nối với chuyên gia phù hợp khi bạn cần một người đồng hành.' },
  { label: 'Căng thẳng', eyebrow: 'Chậm lại một nhịp', title: 'Tìm lại khoảng thở bình yên.', description: 'Những bài tập ngắn và tài nguyên tự chăm sóc giúp bạn trở về với hiện tại.' },
  { label: 'Sang chấn', eyebrow: 'Một không gian an toàn', title: 'Bạn không cần đi qua một mình.', description: 'MentalBridge đồng hành kín đáo và gợi ý hỗ trợ chuyên môn theo đúng mức độ bạn cần.' },
  { label: 'Kiệt sức', eyebrow: 'Lắng nghe cơ thể và tâm trí', title: 'Nghỉ ngơi cũng là một bước tiến.', description: 'Theo dõi những thay đổi nhỏ để nhận ra khi nào bạn cần chậm lại và tìm sự hỗ trợ.' },
] as const
...
export default function Hero() {
  ...
  const orbitPositions = useMemo(() => calculateOrbitPositions(heroTopics.length), [])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const parallaxNodes = Array.from(scene.querySelectorAll<HTMLElement>('.orbit-topic-parallax'))
      if (!halo) return
      ...
      media.add({
        ...
        const lightX = spotlightRef.current ? gsap.quickTo(spotlightRef.current, 'x', { duration: .35, ease: 'power2.out' }) : null
        const lightY = spotlightRef.current ? gsap.quickTo(spotlightRef.current, 'y', { duration: .35, ease: 'power2.out' }) : null
        const lightOpacity = spotlightRef.current ? gsap.quickTo(spotlightRef.current, 'opacity', { duration: .3, ease: 'power2.out' }) : null
        const topicTweens = parallaxNodes.map((item, index) => ({
          x: gsap.quickTo(item, 'x', { duration: .65, ease: 'power3.out' }),
          y: gsap.quickTo(item, 'y', { duration: .65, ease: 'power3.out' }),
          depth: 14 + (index % 3) * 7,
        }))
        ...
        const resetPointer = () => {
          lightOpacity?.(0)
          topicTweens.forEach(item => { item.x(0); item.y(0) })
        }
  ...
  useEffect(() => {
    ...
    import('gsap').then(({ default: gsap }) => {
      ...
      const tween = gsap.fromTo(
        content.children,
        { opacity: 0, y: 14 },
        { opacity: 1, y: 0, duration: .62, stagger: .065, ease: 'power3.out', clearProps: 'transform,opacity' },
      )
      cleanupTween = () => {
        tween.kill()
  ...
  const selectTopic = (index: number) => {
    ...
    import('gsap').then(({ default: gsap }) => {
      ...
      const selected = sceneRef.current?.querySelector<HTMLElement>(`.orbit-topic-${index + 1}`)
      const allTopics = sceneRef.current?.querySelectorAll<HTMLElement>('.orbit-topic')
      ...
      const timeline = gsap.timeline({
        ...
        onComplete: () => {
          transitionRef.current = false
          if (topicTimelineRef.current === timeline) topicTimelineRef.current = null
        },
        onInterrupt: () => {
          gsap.set(content.children, { clearProps: 'transform,opacity' })
  ...
  return (
    <section className="hero hero-orbit" aria-labelledby="hero-title">
      <StarfieldBackground className="hero-starfield" />
        <div
          className="wrap hero-orbit-wrap"
          ref={sceneRef}
      >
        <div className="hero-pointer-light" aria-hidden="true" ref={spotlightRef} />
        ...
        <div ref={orbitTopicsRef} className="orbit-topics" aria-label="Chọn chủ đề bạn đang quan tâm">
          {heroTopics.map((item, index) => {
            ...
            return (
              <button
                ...
                onBlur={event => {
                  if (!event.currentTarget.contains(event.relatedTarget)) resumeOrbit()
                }}
                style={{
                  '--orbit-x': `${position.x}%`,
                  '--orbit-y': `${position.y}%`,
        ...
        <div className="hero-orbit-content" ref={contentRef} aria-live="off">
          ...
          <div className="hero-actions">
            <MagneticButton>
              <Link href="/assessment/anonymous" className="btn btn-primary" data-cursor="action">Bắt đầu sàng lọc miễn phí</Link>
            </MagneticButton>
            <a href="#journey" className="hero-text-link">Xem cách hoạt động <span aria-hidden="true">→</span></a>
          </div>
        </div>
