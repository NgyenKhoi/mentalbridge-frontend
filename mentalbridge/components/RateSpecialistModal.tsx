...
export default function RateSpecialistModal({
  appointment,
  onClose,
  onSubmit,
}: Props) {
  ...
  const submitReview = () => {
    ...
    window.setTimeout(
      () =>
        onSubmit({ rating, highlights, comment: comment.trim(), anonymous }),
      650,
    )
  }
        ...
        >
          <header className="rate-specialist-header">
            <div>
              ...
              <p>
                Phản hồi của bạn giúp chuyên gia cải thiện chất lượng đồng hành.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="rate-specialist-content">
            <section
              className="rate-specialist-summary"
              aria-label="Phiên tư vấn được đánh giá"
            >
              ...
              <div>
                ...
                <p>
                  {formattedDate} · {appointment.time} · {appointment.type}
                </p>
              </div>
            ...
            <section
              className={`rate-specialist-score ${error ? 'has-error' : ''}`}
            >
              <span>
                Mức độ hài lòng <b>*</b>
              </span>
              <div
                className="rate-specialist-stars"
                role="radiogroup"
            ...
            <fieldset className="rate-specialist-highlights">
              <legend>
                Điều bạn đánh giá cao <small>Chọn nhiều mục nếu phù hợp</small>
              </legend>
              <div>
                {HIGHLIGHTS.map((item) => (
                  <button
                    type="button"
                    className={highlights.includes(item) ? 'selected' : ''}
                    key={item}
                    onClick={() => toggleHighlight(item)}
                  >
                    <i>{highlights.includes(item) ? '✓' : '+'}</i>
                    {item}
                  </button>
                ))}
              </div>
            ...
            <label className="rate-specialist-comment">
              <span>
                Nhận xét của bạn <small>Không bắt buộc</small>
              </span>
              <textarea
                rows={4}
                maxLength={500}
                ...
                placeholder="Điều gì khiến phiên tư vấn hữu ích với bạn?"
              />
            ...
            <label className="rate-specialist-anonymous">
              ...
              <i aria-hidden="true" />
              <span>
                <strong>Đăng đánh giá ẩn danh</strong>
                <small>
                  Tên và ảnh đại diện của bạn sẽ không xuất hiện công khai.
                </small>
              </span>
            </label>

            <aside className="rate-specialist-privacy">
              <i>i</i>
              <p>
                <strong>Chỉ chia sẻ trải nghiệm dịch vụ</strong>
                <span>
                  Không đưa thông tin sức khỏe, nội dung riêng tư hoặc chi tiết
                  phiên tư vấn vào nhận xét công khai.
                </span>
              </p>
            </aside>
          </div>
          ...
          <footer className="rate-specialist-footer">
            ...
            <button
              type="button"
              className="rate-specialist-submit"
