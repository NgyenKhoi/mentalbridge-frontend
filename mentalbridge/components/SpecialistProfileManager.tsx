...
const tabs: { id: ProfileTab; label: string; description: string }[] = [
  {
    ...
    description: 'Tên, giới thiệu và ngôn ngữ',
  },
  {
    id: 'expertise',
    label: 'Chuyên môn',
    description: 'Lĩnh vực và chứng chỉ',
  },
  {
    id: 'consultation',
    label: 'Thiết lập tư vấn',
    description: 'Hình thức, thời lượng và phí',
  },
]
...
export default function SpecialistProfileManager() {
  ...
  const completion = useMemo(() => {
    ...
    const completed =
      ...
      Number(profile.specialties.length > 0) +
      Number(profile.certificates.length > 0) +
      Number(profile.formats.length > 0)
  ...
  const toggleArrayValue = (
    key: 'languages' | 'specialties' | 'formats',
    value: string,
  ) => {
    const next = profile[key].includes(value)
  ...
  return (
    <section className="sp-manager">
      <header className="sp-heading">
        <div>
          <span className="sp-eyebrow">Hồ sơ chuyên gia</span>
          <h1>Hồ sơ nghề nghiệp của bạn</h1>
          <p>
            Quản lý thông tin khách hàng nhìn thấy trước khi lựa chọn và đặt
            lịch tư vấn với bạn.
          </p>
        </div>
        <button
          type="button"
          className="sp-preview-button"
          onClick={() => setPreviewOpen(true)}
        >
          <span aria-hidden="true">◉</span> Xem hồ sơ công khai
        </button>
      </header>

      <div className="sp-identity">
        <div className="sp-avatar" aria-label="Ảnh đại diện hiện tại">
          TH<span title="Đã xác minh">✓</span>
        </div>
        <div className="sp-identity-copy">
          <div className="sp-name-line">
            <h2>{profile.name}</h2>
            <span className="sp-verified">Đã xác minh</span>
          </div>
          <p>
            {profile.title} <i /> {profile.experience || '0'} năm kinh nghiệm
          </p>
          <button
            type="button"
            className="sp-text-button"
            onClick={() =>
              setNotice('Tính năng tải ảnh sẽ được kết nối với API hồ sơ.')
            }
          >
            Thay ảnh đại diện
          </button>
        ...
        <div className="sp-availability">
          <Toggle
            ...
            onChange={() =>
              update('acceptingClients', !profile.acceptingClients)
            }
            label="Nhận khách hàng mới"
          />
          <div>
            <strong>
            ...
            <small>Hiển thị trên hồ sơ công khai</small>
          </div>
        </div>
      </div>
      ...
      <div className="sp-workspace">
            ...
            {activeTab === 'information' && (
              <>
                <div className="sp-section-head">
                  ...
                  <p>
                    Các thông tin này xuất hiện trên trang tìm kiếm và hồ sơ
                    chuyên gia.
                  </p>
                </div>
                ...
                <label className="sp-textarea">
                  ...
                  />
                  <small>{profile.bio.length}/700 ký tự</small>
                </label>
                <fieldset className="sp-choice-group">
                  <legend>Ngôn ngữ tư vấn</legend>
                  <div>
            ...
            {activeTab === 'expertise' && (
              <>
                <div className="sp-section-head">
                  <div>
                    <span>02</span>
                    <h2>Chuyên môn &amp; chứng chỉ</h2>
                  </div>
                  <p>
                    Chọn đúng thế mạnh để khách hàng phù hợp dễ dàng tìm thấy
                    bạn.
                  </p>
                </div>
                <fieldset className="sp-choice-group">
                  <legend>Lĩnh vực đồng hành</legend>
                  <div>
                    {specialtyOptions.map((item) => (
                      <button
                        ...
                        onClick={() => toggleArrayValue('specialties', item)}
                      >
                        <span>
                          {profile.specialties.includes(item) ? '✓' : '+'}
                        </span>
                ...
                <label className="sp-textarea">
                  <span>Phương pháp đồng hành</span>
                  <textarea
                    rows={5}
                ...
                <div className="sp-certificates">
                  <div className="sp-field-title">
                    <span>Đào tạo &amp; chứng chỉ</span>
                    <small>Chỉ thêm thông tin có thể xác minh</small>
                  </div>
                  {profile.certificates.map((certificate, index) => (
                    <div
                      className="sp-certificate"
                  ...
                  <div className="sp-add-certificate">
                    <input
                      value={certificateDraft}
                      onChange={(event) =>
                        setCertificateDraft(event.target.value)
                      }
                      onKeyDown={(event) => {
                        if (event.key === 'Enter') {
            ...
            {activeTab === 'consultation' && (
              <>
                <div className="sp-section-head">
                  <div>
                    <span>03</span>
                    <h2>Thiết lập tư vấn</h2>
                  </div>
                  <p>
                    Thiết lập thông tin dịch vụ hiển thị khi khách hàng chuẩn bị
                    đặt lịch.
                  </p>
                </div>
                <fieldset className="sp-format-list">
                  ...
                  {formatOptions.map((format) => (
                    <div key={format}>
                      <span className="sp-format-icon" aria-hidden="true">
                        {format === 'Video call'
                          ? '▣'
                          : format === 'Điện thoại'
                            ? '⌕'
                      ...
                      <div>
                        ...
                        <small>
                          {format === 'Video call'
                            ? 'Tư vấn trực tuyến trên MentalBridge'
                            : format === 'Điện thoại'
                              ? 'Trao đổi bằng cuộc gọi thoại'
                              : 'Gặp trực tiếp tại địa điểm của bạn'}
                        </small>
                      </div>
                      <Toggle
                        checked={profile.formats.includes(format)}
                        onChange={() => toggleArrayValue('formats', format)}
                        label={format}
                      />
                    </div>
                  ))}
                ...
                <div className="sp-fields two-columns">
                  <label>
                    <span>Phí tư vấn mỗi phiên</span>
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        update('responseTime', event.target.value)
                      }
                    >
                      <option>Trong vòng 12 giờ</option>
                      <option>Trong vòng 24 giờ</option>
                      <option>Trong vòng 48 giờ</option>
                    </select>
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      disabled={!profile.formats.includes('Tại phòng tư vấn')}
                    />
                  </label>
                </div>
              </>
        ...
        <aside className="sp-public-summary">
          <div className="sp-summary-label">
            <span>Bản xem trước</span>
            <i className={profile.acceptingClients ? 'is-online' : ''}>
              {profile.acceptingClients ? 'Đang nhận lịch' : 'Tạm dừng'}
            </i>
          </div>
          ...
          <p className="sp-public-title">
            {profile.title || 'Chức danh chuyên môn'}
          </p>
          <div className="sp-public-meta">
            <span>{profile.experience || '0'} năm kinh nghiệm</span>
            <span>{profile.location || 'Chưa cập nhật'}</span>
          </div>
          <div className="sp-public-tags">
          ...
          <dl>
            ...
            <div>
              <dt>Hình thức</dt>
              <dd>{profile.formats.join(' · ') || 'Chưa cập nhật'}</dd>
            </div>
            <div>
              <dt>Phí tư vấn từ</dt>
              <dd>
                {fee}đ / {profile.duration} phút
              </dd>
            </div>
          </dl>
          <button type="button" disabled={!profile.acceptingClients}>
            Đặt lịch tư vấn <span>→</span>
          </button>
          <small className="sp-preview-note">
            Thay đổi chỉ xuất hiện với khách hàng sau khi bạn lưu hồ sơ.
          </small>
        </aside>
      </div>

      <footer className={`sp-savebar ${dirty ? 'has-changes' : ''}`}>
        <div>
          <span>
            {dirty ? '● Có thay đổi chưa lưu' : '✓ Mọi thay đổi đã được lưu'}
          </span>
          <small>Cập nhật lần cuối hôm nay, 09:42</small>
        </div>
        <div>
          <button
            type="button"
      ...
      {previewOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Xem trước hồ sơ công khai"
        >
          <button
          ...
          <div className="sp-modal-content">
            ...
            <span className="sp-modal-kicker">
              HỒ SƠ CÔNG KHAI · BẢN XEM TRƯỚC
            </span>
            <div className="sp-modal-profile">
              <div className="sp-public-avatar">
                TH<span>✓</span>
              </div>
              <div>
            ...
            <div className="sp-modal-stats">
              <div>
                ...
                <strong>{profile.experience} năm</strong>
              </div>
              <div>
                <small>Hình thức</small>
                <strong>{profile.formats.length} lựa chọn</strong>
              </div>
              <div>
                <small>Phí từ</small>
                <strong>{fee}đ</strong>
