...
challenges, rate limits, and refresh-session revocation. The frontend owns only
...
- Verification resend and recovery request routes always return the same empty
  accepted response for eligible and ineligible accounts. A rate-limited result
  contains only the safe retry interval.
...
- Unit and browser tests cover invalid input, dependency-safe errors, rate-limit
  feedback, cookie cleanup, challenge URL cleanup, resend, reset, and change.
...
| Command | Result |
| --- | --- |
| `npm run quality` | Passed: format, lint, typecheck, three OpenAPI contract checks, 170 unit tests, and production build with 44 routes |
...
No automated test calls a live Identity or email provider. The production build
